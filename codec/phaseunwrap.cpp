...
void unwrappatch(cv::Mat phase, cv::Mat quality, cv::Mat mask, cv::Mat phOffset, float threshold, cv::Point ptStart, int borderX, int borderY, int stepX, int stepY){
    ...
    for(int r = ptStart.y; r != borderY-1; r += stepY){
        ...
        for(int c = ptStart.x; c != borderX-1; c += stepX){
            ...
            if((mask.at<unsigned char>(r,c) == 255) && (quality.at<float>(r,c) <= threshold)){
                ...
                if(quality.at<float>(r,c-stepX) == VAL_UNWRAPPED){
                    ...
                    if(quality.at<float>(r-stepY,c) == VAL_UNWRAPPED){
                        ...
                        if((mask.at<unsigned char>(r,c+stepX) == 255) || (mask.at<unsigned char>(r+stepY,c) == 255) )
...
void unwrap(cv::Mat phase, cv::Mat quality, cv::Mat mask, const std::vector<float> thresholds)
{
    ...
    for(unsigned int i = 0; i < thresholds.size(); i++){
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, 1, 1, -1, -1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, 1, phase.rows, -1, 1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, phase.cols, 1, 1, -1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, phase.cols, phase.rows, 1, 1);
    }
